...
`type` must be equal to `extension` (there will more template available in the future for instance to create plugin to connect to other chat platform than Twitch). `package` (`module-name`) will be the Java package (the Java module name respectively) of the plugin and as such should respect Java package naming (no '.' or '-' for instance).
