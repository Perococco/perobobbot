...
public class StartProgramMutation implements Mutation<ManagerState> {
    ...
    public @NonNull ManagerState mutate(@NonNull ManagerState state) {
        ...
        try {
            program.start();
            final ImmutableSet<String> enabled = SetTool.add(state.getNamesOfEnabledPrograms(),programName);
