...
public class ProgramManagerFactory {
    ...
    private CommandBundleFactory<ProgramManager> bundleFactory;
    ...
    private void setupChatCommands() {
        ...
        this.bundleFactory = CommandBundleFactory.with(
                chatController,
                p -> ImmutableList.of(
                        complex("pm",
                                simple("list", policy.createAccessPoint(new ListPrograms(p, io))),
                                simple("start", policy.createAccessPoint(new StartProgram(p))),
                                simple("stop", policy.createAccessPoint(new StopProgram(p))),
                                simple("start-all", policy.createAccessPoint(ctx -> p.startAll())),
                                simple("stop-all", policy.createAccessPoint(ctx -> p.stopAll()))
                        )
                )
        );
    }
    ...
    private void createProgramManager() {
        this.programManager = new PerococcoProgramManager(loadPrograms(), bundleFactory);
    }
